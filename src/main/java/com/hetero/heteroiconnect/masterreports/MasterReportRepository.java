...
public class MasterReportRepository {
	...
	public List<EmployeeMasterDetailsDTO> getReportingMasterDetails(String reportingIdsCsv) {
		...
		// Parse comma-separated reporting IDs
	    List<String> reportingIdList = new ArrayList<>();
	    if (reportingIdsCsv != null && !reportingIdsCsv.isEmpty()) {
	        reportingIdList = Arrays.stream(reportingIdsCsv.split(","))
	                                .map(String::trim)
	                                .filter(s -> !s.isEmpty())
	                                .collect(Collectors.toList());
	    }

	    // Add reporting ID condition
	    if (!reportingIdList.isEmpty()) {
	        String placeholders = reportingIdList.stream().map(id -> "?").collect(Collectors.joining(","));
	        appendCondition(whereClause, "A.EMPLOYEESEQUENCENO IN (" + placeholders + ")");
	        params.addAll(reportingIdList);
	    }
		...
		String basicDetailsQuery = "SELECT A.EMPLOYEESEQUENCENO 'Emp ID', A.CALLNAME 'NAME', STATUS.NAME 'STATUS', "
				...
				+ "BU.NAME 'DIVISION', IFNULL(DES.NAME,'') 'DESIGNATION', IFNULL(DEP.NAME,'') 'DEPARTMENT', "
				+ "IFNULL(MIS.MIS_DEPARTMENT,'') 'MIS_DEPARTMENT', "
				+ "CONCAT(MN.EMPLOYEESEQUENCENO,'-',MN.CALLNAME) AS 'Reportee', MS.NAME 'Reportee_Status' "
				+ "FROM HCLHRM_PROD.TBL_EMPLOYEE_PRIMARY A "
				+ "LEFT JOIN HCLADM_PROD.TBL_STATUS_CODES STATUS ON A.STATUS=STATUS.STATUS "
				+ "LEFT JOIN HCLADM_PROD.TBL_BUSINESSUNIT BU ON A.COMPANYID=BU.BUSINESSUNITID "
				+ "LEFT JOIN hclhrm_prod.tbl_employee_profile PROFILE ON A.EMPLOYEEID=PROFILE.EMPLOYEEID "
				...
				+ "LEFT JOIN test.tbl_mis_department_mapping MIS ON A.EMPLOYEESEQUENCENO = MIS.EMPLOYEEID "
				+ "LEFT JOIN hcladm_prod.tbl_costcenter co ON co.COSTCENTERID=a.COSTCENTERID " + " " + finalWhereClause
				+ " " + "GROUP BY a.employeeid order by  MN.EMPLOYEESEQUENCENO  ";
...
//				.collect(Collectors.toList());
		...
		List<EmployeeMasterDetailsDTO> sortedList = employeeMap.values().stream()
		        .sorted(Comparator
		            .comparing(EmployeeMasterDetailsDTO::getReportee, Comparator.nullsLast(Comparator.naturalOrder()))
		            .thenComparingInt(e -> Integer.parseInt(e.getEmpId()))
		        )
		        .collect(Collectors.toList());
		return sortedList;
	...
	public List<Integer> getAllSubordinates(int managerSeq) {
        return jdbcTemplate.query(
            "CALL procedure.GetAllSubordinates(?)",
            new Object[]{managerSeq},
            (ResultSet rs) -> {
                List<Integer> employeeIds = new ArrayList<>();
                while (rs.next()) {
                    employeeIds.add(rs.getInt("employeesequenceno"));
                }
                return employeeIds;
            }
        );
	 ...
	 public List<String> getReportingEmployees(int managerSeq) {
	        String sql = "SELECT CONCAT(p.employeesequenceno,'-',p.callname) AS ReportingName " +
	                     ...
	                     "LEFT JOIN hclhrm_prod.tbl_employee_primary h ON D.managerid = h.EMPLOYEEID " +
	                     "WHERE h.employeesequenceno = ? AND p.status = 1001 " +
	                     "GROUP BY p.employeesequenceno  ORDER BY P.CALLNAME";

	        return jdbcTemplate.queryForList(sql, new Object[]{managerSeq}, String.class);
	    }
