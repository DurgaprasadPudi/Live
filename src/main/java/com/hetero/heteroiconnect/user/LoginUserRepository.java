...
public interface LoginUserRepository extends JpaRepository<LoginUser, Long> {
	...
	//// for production 
	 ...
	 @Query(value = "SELECT A.EMPLOYEESEQUENCENO 'EmpID', A.COMPANYID 'BUID',lg.password iconnectpassword,"
	 		+ " CONCAT(md5(:userName),md5(:password)) PWD,"
	 		+ " A.EMPLOYEEID HRMSEMPLOYEEID,A.CALLNAME 'NAME',"
	 		+ " A.FIRSTNAME 'FIRSTNAME',A.LASTNAME 'LASTNAME', "
	 		+ " STATUS.NAME 'STATUS',co.NAME 'COSTCENTER', "
	 		+ " GEN.NAME 'GENDER', GEN.GENDER 'GENDERID', EMPLO.NAME 'EMPLOYMENT TYPE', "
	 		+ " IFNULL(STROKE.NAME,'--') 'INCREMENT TYPE',  "
	 		+ " IFNULL(IF(STATUS.NAME='ACTIVE','--',HR.DATE),'') 'LWD', "
	 		...
	 		+ " MS.NAME 'Reportee_Status', BANK.BANKID, IFNULL(BANK.BANKNAME,   'NA') 'BANKNAME', "
	 		+ " IFNULL(BIFSC.BANKIFC,'NA') 'IFSC', IFNULL(OTHER.ACCOUNTNO,'NA') 'ACCOUNTNO',   "
	 		+ " IFNULL(PRO.EMAIL,'') 'PROEMAIL', IFNULL(D.EMAIL,'') 'PEMAIL', IFNULL(OTHER.PFNO,     'NA') PFNO,   "
	 		+ " IFNULL(OTHER.ESINO,    'NA')ESINO, IFNULL(OTHER.PFUAN,    'NA')PFUAN, IFNULL(EMPST.STATE,'') STATE,  "
	 		+ " IFNULL(HQLOC.NAME,'') HQ, IFNULL(RLOC.NAME,'') REGION, IFNULL(PRO.MOBILE,'') 'PROFESSIONAL_MOBILE',   "
	 		+ " IFNULL(D.MOBILE,'')'PERSONAL_MOBILE', IFNULL(D.MOBILE,'')'PERSONAL_PHONE',   "
	 		...
	 		+ " IFNULL(COMMLOC.NAME,'') COMMCITY,   IFNULL(COMMSTATELOC.NAME,'')COMMSTATE, "
	 		+ " COMMSTATELOC.LOCATIONID COMMSTATE_LOCATIONID,COMMLOC.LOCATIONID COMMCITY_LOCATIONID,IFNULL(D.COMMUNICATIONZIP,'') COMM_ZIP,   "
	 		...
	 		+ " PSLOC.LOCATIONID PERSTATE_LOCATIONID,PLOC.LOCATIONID PERCITY_LOCATIONID,IFNULL(INFO.PASSPORTNO,'') 'PASSPORTNO', "
	 		+ " IFNULL(INFO.AADHAARCARDNO,'') 'AADHAARCARDNO',   IFNULL(INFO.AADHAARUID,'NA') 'AADHAARUID', "
	 		+ " IFNULL(INFO.AADHAARNAME,'NA') 'AADHAARNAME',   "
	 		+ " IFNULL(INFO.PAN,'NA') 'PAN', IFNULL(IMPRS.assetpropertyvalue,'0.00') 'IMPRESTAMT',   "
	 		+ " QUALI.QUALIFICATION 'Education_Details', IFNULL(PREV.PREV,'0') 'Prev_Exp',CUREXP.Exp 'Cur_Exp' ,"
	 		+ " ice.ADDRESS ICEADDRESS,ice.ADDRESS2 ICEADDRESS2,ice.ADDRESS3 ICEADDRESS3,ice.ADDRESS4 ICEADDRESS4,"
	 		+ " ice.MOBILE ICEMOBILE,ice.FIRSTNAME ICE_FIRSTNAME,ice.LASTNAME ICE_LASTNAME,CONCAT(ice.FIRSTNAME,' ',ice.LASTNAME) CONTACTPERSONNAME,"
	 		+ " ICESTATELOC.NAME 'ICESTATE',ICELOC.NAME 'ICECITY',IFNULL(ICESTATELOC.LOCATIONID,'0') 'ICE_STATE_ID',"
	 		+ " IFNULL(ICELOC.LOCATIONID,'0') 'ICE_CITY_ID',rel.RELATIONID,rel.RELATIONNAME, "
	 		+ " CONCAT(SUBSTRING_INDEX((TOTALPREV+TIMESTAMPDIFF(MONTH, DATEOFJOIN, NOW()+1))/12, '.',1),'.',ROUND((CONCAT('0.',SUBSTRING_INDEX((TOTALPREV+TIMESTAMPDIFF(MONTH,DATEOFJOIN, NOW()+1))/12, '.',-1)))*12,0)) TOTALEXP,"
	 		+ " NOW() TODAYDATE,BU.CODE LOCATION,"
	 		+ " CASE WHEN A.EMPLOYEESEQUENCENO IN (11110,10717) "
	 		+ " THEN BU.CALLNAME "
	 		+ " WHEN BU.BUSINESSUNITID IN (15,16,33,34) "
	 		+ " THEN 'ASSAM' ELSE BU.CALLNAME END AS SUBLOCATION,"
	 		+ " DATEDIFF(NOW(),PROFILE.DATEOFJOIN)+1 WORKINGDAYS,IF(IFNULL(NT.EMPLOYEEID,'0')=0,'FALSE','TRUE') NOTICE,"
	 		+ " IF(SUBSTRING_INDEX(SUBSTRING_INDEX(procedure.GET_HOD_ID_NAME(A.EMPLOYEESEQUENCENO), '~', 2),'~', 1)='30546','TRUE',"
	 		+ " if('30546'=A.EMPLOYEESEQUENCENO,'TRUE' ,'FALSE')) WORKSHEET  "
	 		
	 		+ " , if(lg.datecreated=lg.lupdate,'1','0') as FIRSTLOGIN  "  // attached for first login
	 		+ " FROM HCLHRM_PROD.TBL_EMPLOYEE_PRIMARY A LEFT JOIN HCLADM_PROD.TBL_STATUS_CODES STATUS ON 	"
	 		+ " A.STATUS=STATUS.STATUS LEFT JOIN HCLADM_PROD.TBL_BUSINESSUNIT BU 	"
	 		+ " ON A.COMPANYID=BU.BUSINESSUNITID LEFT JOIN hclhrm_prod.tbl_employee_personal_contact D 	ON "
	 		+ " A.EMPLOYEEID=D.EMPLOYEEID LEFT JOIN HCLLCM_PROD.TBL_LOCATION PLOC 	ON D.PERMANENTLOCATIONID=PLOC.LOCATIONID "
	 		...
	 		+ " LEFT JOIN ( SELECT EMPLOYEEID,MAX(LASTWORKINGDATE) DATE FROM HCLHRM_PROD.TBL_EMPLOYEE_HRACTIONS GROUP BY EMPLOYEEID ) HR "
	 		+ " ON A.EMPLOYEEID=HR.EMPLOYEEID LEFT JOIN (  "
	 		+ "  select  assetpropertyvalue,issueddate,returneddate,employeeid from  hclhrm_prod.tbl_employee_assets where assetid=16 "
	 		+ " and status=1001 group by employeeid ) IMPRS " // subquery
	 		+ "ON  IMPRS.employeeid=A.employeeid  "
	 		+ " LEFT JOIN ( SELECT EMPLOYEEID,GROUP_CONCAT(QUALIFICATIONID,'-',YEAROFPASSING) QUALIFICATION FROM   "
	 		+ " hclhrm_prod.tbl_employee_education_details GROUP BY EMPLOYEEID ) QUALI ON A.EMPLOYEEID=QUALI.EMPLOYEEID   "
	 		+ " LEFT JOIN ( SELECT EMPLOYEEID,CONCAT(SUBSTRING_INDEX(TIMESTAMPDIFF(MONTH,DATEOFJOIN, NOW()+1)/12,'.',1),'.', "
	 		+ " ROUND((CONCAT('0.',SUBSTRING_INDEX(TIMESTAMPDIFF(MONTH, DATEOFJOIN, NOW()+1)/12,'.',-1)))*12,0)) 'Exp' "
	 		+ " FROM   HCLHRM_PROD.TBL_EMPLOYEE_PROFILE GROUP BY EMPLOYEEID ) CUREXP ON A.EMPLOYEEID=CUREXP.EMPLOYEEID   "
	 		+ " LEFT JOIN ( SELECT EMPLOYEEID,CONCAT(SUBSTRING_INDEX(SUM(EXPERIENCE)/12, '.',1),'.',"
	 		+ " ROUND((CONCAT('0.',SUBSTRING_INDEX(SUM(EXPERIENCE)/12, '.',-1)))*12,0)) PREV,SUM(EXPERIENCE) TOTALPREV "
	 		+ " FROM   hclhrm_prod.tbl_employee_experience_details GROUP BY EMPLOYEEID ) PREV ON A.EMPLOYEEID=PREV.EMPLOYEEID   "
	 		...
	 		+ " left join hclhrm_prod.tbl_employee_login lg on lg.employeeid=a.employeeid  "
	 		+ " left join tbl_employee_ice_contact ice on ice.employeeid=a.employeeid "
	 		+ " LEFT JOIN HCLLCM_PROD.TBL_LOCATION ICELOC ON ICE.LOCATIONID=ICELOC.LOCATIONID "
	 		+ " LEFT JOIN HCLLCM_PROD.TBL_LOCATION ICESTATELOC ON ICELOC.PARENT=ICESTATELOC.LOCATIONID "
	 		+ " LEFT JOIN  hclhrm_prod_others.tbl_iconnect_profile_pics pc on pc.employeeid=A.EMPLOYEESEQUENCENO "
	 		+ " LEFT JOIN hcladm_prod.tbl_relations rel on rel.RELATIONID=ice.RELATIONID "
	 		+ " LEFT JOIN test.tbl_employee_notice nt on nt.employeeid=A.EMPLOYEESEQUENCENO   "
	 		+ "  WHERE  lg.EMPLOYEECODE=:userName and lg.password = MD5(:password) and a.status in(1001,1092,1401)   "
	 		+ " group by a.employeeid ORDER BY A.EMPLOYEESEQUENCENO ;", nativeQuery = true)
